...
| 12 | **Ahenk kimliği sıfırla** | LiderAhenk ajanının yerel kimliğini sıfırlar — kopya tahtaların Pulsar broker'ında kimlik çakışması yaşamaması için: `ahenk.service` durdurulur; `ahenk.conf` içindeki `uid`/`parola`/`sunucu adresi`, `messaging.conf` içindeki Pulsar bağlantı alanları (`pulsar_host`, `pulsar_port`, `tls_trust_certs_file_path`) boşaltılır; `ahenk.db` yerel kayıt veritabanı silinir; `ahenk.log` boşaltılır. **Paket kaldırılmaz, servis `enabled` bırakılır**; kopya tahta ilk açılışında ahenk kendine yeni UUID üretip Lider'e kendi adıyla yeniden kayıt akışına girer. Bu tahtada ahenk yüklü değilse adım sessizce atlanır. **Kısmi geri alma:** yıkımdan önce dosyalar `/var/lib/tiha/state/m12_ahenk_backup/` altına yedeklenir; imaj alınmadan vazgeçilirse Özet'teki "Geri al" yedeği geri yükler ve servisi yeniden başlatır. |
