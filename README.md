...
| 11 | **İmaj için sanitize** | **Geri alınamaz** bir temizlik adımı. İki kategori iş yapar: *(a) tekil kimlik silme* — `machine-id`, SSH host anahtarları (ilk açılışta yeniden üreten servis bırakılır), NetworkManager bağlantıları (WiFi parolaları dahil), DHCP lease'ler, `random-seed`; *(b) yer açma & iz silme* — APT önbelleği, `rc-state` paketleri (`dpkg --purge`), `apt-get autoremove --purge`, systemd journal (1 KB'a vakum), tüm `/var/log`, crash raporları, mail/cron spool kuyrukları, sistem önbellekleri (man/fontconfig/debconf/PackageKit/lightdm/cups), `dpkg-old`/`dpkg-dist`/`ucf-*` yedekleri, kullanılmayan diller (`tr*`, `en*`, `C`, `POSIX` dışındakiler), tüm kullanıcıların `~/.cache`, `~/.local/share/Trash`, kabuk geçmişleri (`.bash_history`, `.zsh_history`, `.viminfo`, `.python_history`…), `/tmp`, `/var/tmp` ve **web tarayıcı önbellek + gezinti verileri** (Firefox, Chrome, Chromium, Edge, Brave, Vivaldi, Opera, Yandex — gezinti geçmişi, çerezler, oturumlar, IndexedDB; yer imleri korunur). Bittiğinde popup ile boşalan disk miktarını bildirir. Tipik kazanım: 500 MB – 1 GB+. **Ahenk kimliği bu adımda dokunulmaz** — onun için 12. adımı çalıştırın. |
| 12 | **Ahenk kimliği sıfırla** | LiderAhenk ajanının yerel kimliğini sıfırlar — kopya tahtaların Pulsar broker'ında kimlik çakışması yaşamaması için: `ahenk.service` durdurulur; `ahenk.conf` içindeki `uid`/`parola`/`sunucu adresi`, `messaging.conf` içindeki Pulsar bağlantı alanları (`pulsar_host`, `pulsar_port`, `tls_trust_certs_file_path`) boşaltılır; `ahenk.db` yerel kayıt veritabanı silinir; `ahenk.log` boşaltılır. **Kazara reboot koruması:** imaj anındaki birincil MAC adresi `/var/lib/tiha/state/imaged-mac` altına yazılır, `ahenk.service` **disable** edilir, ve `tiha-post-image-init.service` (oneshot) kurulur. Bu servis her boot'ta MAC karşılaştırması yapar: aynı MAC ise (orijinal tahta yanlışlıkla yeniden başlatılmış) hiçbir şey yapmaz → ahenk başlamaz → imaj kirletilmez. Farklı MAC ise (klon tahta) ahenk kimliği savunma katmanıyla tekrar boşaltılır, `enable + start` edilir, MAC dosyası güncellenir, klon-tespit servisi kendini disable eder. eta-register backend'e MAC sorgusu attığı için yeni kayıt akışı otomatik tetiklenir. Bu tahtada ahenk yüklü değilse adım sessizce atlanır. **Kısmi geri alma:** yıkımdan önce dosyalar `/var/lib/tiha/state/m12_ahenk_backup/` altına yedeklenir; imaj alınmadan vazgeçilirse Özet'teki "Geri al" yedeği geri yükler, klon-tespit servisini kaldırır ve ahenk'i tekrar `enable + start` eder. |
