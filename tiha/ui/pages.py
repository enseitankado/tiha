...
class ModulePage(Gtk.Box):
    ...
    def _make_field(self, field: dict) -> Gtk.Widget:
        ...
        if kind == "branch_accounts":
            # Özet + ayrı seçim penceresi (okul türü ve branşlar geniş bir
            # alanda seçilir). Değer m01'in beklediği JSON'dur.
            from .branch_picker import BranchAccountsField

            field_widget = BranchAccountsField(default or "")
            field_widget._branch_value = field_widget.value  # type: ignore[attr-defined]
            field_widget._set_branch_selection = field_widget.set_selection  # type: ignore[attr-defined]
            return field_widget
